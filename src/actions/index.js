//User Actions
import { 
    USER_LOGIN_REQUEST, USER_LOGIN_SUCCESS , USER_LOGIN_FAIL , USER_LOGOUT_SUCCESS , USER_REGISTER_REQUEST , USER_REGISTER_SUCCESS , DOMAIN , USER_REGISTER_FAIL
} from "../constants";
  ...
  export const registerUser = (obj , history) => {
    return async (dispatch) => {
      dispatch({ type: USER_REGISTER_REQUEST });
       try{      
        const {data} = await axios.post(`${DOMAIN}/api/register`,{...obj, firstname : '' , lastname : '' , type : 0});
        console.log(data)
        if(data.status){
